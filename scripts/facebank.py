...
def load_facebank(path = cur_path+'facebank'):
    data_path = Path(path)
...
if __name__ == '__main__':
    ...
    detect_model.load_state_dict(
        torch.load(cur_path+'Weights/MobileFace_Net', map_location=lambda storage, loc: storage))
    print('MobileFaceNet face detection model generated')
    detect_model.eval()

    embeddings, names = prepare_facebank(detect_model, path = cur_path+'facebank', tta = True)
